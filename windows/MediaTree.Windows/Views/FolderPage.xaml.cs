...
public sealed partial class FolderPage : Page
{
    ...
    private async System.Threading.Tasks.Task LoadAsync()
    {
        ...
        try
        {
            ...
            var response = await AppServices.Media.Movie.GetMoviesAsync(_mediaRoot, _folderPath, "", sort, 2000, 0);
            var specials = string.IsNullOrWhiteSpace(_folderPath)
                ? new FolderSpecialsResponseDto()
                : await AppServices.Media.Movie.GetFolderSpecialsAsync(_folderPath, _mediaRoot, includeMovies: true);
    ...
    private async System.Threading.Tasks.Task SelectSpecialsAsync()
    {
        try
        {
            ...
            {
                await AppServices.Media.Movie.SetFolderSpecialsAsync(_folderPath, _mediaRoot, true);
                var specials = await AppServices.Media.Movie.GetFolderSpecialsAsync(_folderPath, _mediaRoot, includeMovies: true);
                _showSpecials = specials.ShowSpecials;
                _specialCount = specials.SpecialCount;
