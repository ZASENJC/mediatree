...
- 文件夹标题支持展示 TMDB 标题图，优先使用中文 Logo，其次使用英文 Logo；图片不可用时自动回退到文字标题
- 放大文件夹标题并精简首屏控件，移除重复的 `Folder` 标签和页内“返回首页”按钮
- 单集卡片优先展示单集剧照，缺图时回退到文件夹横版背景；手机保持两列，桌面根据空间自适应排列
- 共享横版背景可在单集卡片之间复用浏览器缓存，并稳定离屏 16:9 卡片尺寸，减少重复下载和滚动跳动

### 安全更新

- 将 `pydantic-settings` 升级到 2.14.2、`python-multipart` 升级到 0.0.31、`Pillow` 升级到 12.3.0，纳入当前安全修复

### 发布类型

- 需要完整 Docker 镜像更新，以安装修复后的 Python 依赖
- Docker Compose 用户请在宿主机执行 `docker compose pull && docker compose up -d`；新安装使用的 `zasenjc/mediatree:latest` 会刷新到最新应用基线
...
- 设置页改为右侧抽屉，在当前页面上方打开，不再替换主页面路由
- 放宽设置分类栏，让标签完整显示，同时保持文字小字号并加宽选中态阴影底框
- 设置的液态玻璃效果只保留在抽屉面板上，打开设置时不再模糊或压暗底下的页面
- 设置抽屉底部新增轻量 GitHub 项目归属链接
- 移除旧的 Javdatabase 全局启用开关，Javdatabase 现在只通过刮削器选择和插件注册控制
- 刮削器设置只保留 `TMDB 读访问令牌`，并提供令牌申请教程链接

### 媒体库

- 首页改为将“继续观看”以紧凑横向列表展示在媒体库海报网格上方，区块标题使用更轻量的小字号，并移除原来的切换入口
- 稳定首页媒体库初始布局，避免海报行在页面渲染完成过程中向下滑动
- 首页媒体库在排序或布局变化导致海报移动时，会以接近 60fps 的节奏播放更自然的位置过渡动画

### 播放

- 详情页打开后会自动播放，并让播放器适配首屏高度；标题、元数据和图库继续放在播放器下方
- 手机和触控设备上的详情页改用普通上下排列播放器布局，不再强制套用首屏适配高度
- 保持普通嵌入播放器默认不自动播放，只有详情页等明确传入 `autoPlay` 的页面才会启用

### 维护

- 更新前端构建工具链和间接开发依赖，清除当前 `npm audit` 报告的问题

### 发布类型

- 应用包级更新；不需要完整 Docker 镜像更新
- Docker Compose 用户可在设置页更新；新安装使用的 `zasenjc/mediatree:latest` 会刷新到最新应用基线
...
- 播放页浏览器标签标题会随播放状态显示：播放中为 `▶ 当前播放标题 - MediaTree`，暂停或结束为 `⏸ 当前播放标题 - MediaTree`
- 用户停留在播放页时持续保留播放状态标题，只有离开播放页才恢复原站点标题
- 剧集/选集播放时，浏览器标签标题优先使用当前集标题

### 更新

- 设置页更新卡片会将旧应用包版本显示为 `回滚此版本`，新应用包版本显示为 `下载并更新`
- 回滚与更新操作的确认框、进度和错误提示文案保持一致
...
- 刮削器缓存有效期和 Javdatabase 请求间隔改为后端内部策略，不再通过设置页或环境变量调节
- 手动扫描、重新刮削、手动刮削应用和选择结果应用都会绕过刮削缓存，确保用户主动刷新能拿到最新数据
- 空刮削结果不再写入缓存，读取到历史空缓存时会自动删除，避免旧空结果挡住后续补齐的数据
- Javdatabase 的搜索、直连、模糊匹配和详情请求统一使用内部 3 秒请求间隔
...
- 本版本需要完整 Docker 镜像更新，因为 Docker 镜像 metadata、更新基线、发布链和运行时刮削策略默认值发生变化
...
- 应用包更新成功并完成重启后，会自动清理更旧的应用包目录和已下载归档
- 保留当前应用包和一个有效上一版，确保回滚和启动失败 fallback 仍然可用
- 清理时会跳过 `data/releases/` 下未知用途的目录，避免误删用户或未来运行时文件

### 安全与依赖

- 更新生产环境 React Router 依赖链，清除 `npm audit --omit=dev` 报告的 same-origin redirect advisory

### 发布流水线

- 应用包发布现在也会刷新 DockerHub `zasenjc/mediatree:latest`，让新安装用户直接获得最新应用基线
- 完整镜像发布仍会同时推送版本化 Docker tag 和 `latest`

### 品牌与文档

- 登录页 LOGO 和网页图标恢复为随应用包发布的本地资源，不再依赖远程图片 URL
- README、compose 示例和 wiki 补充 Docker socket 挂载说明，明确它只用于可选的完整镜像更新
...
- 认证现在默认失败关闭，空的 `AUTH_USER` / `AUTH_PASS` 不再代表允许匿名访问
- 新增首次打开时创建管理员账号流程，使用签名 session token，并以哈希形式保存密码
- 未登录时禁止首次媒体库初始化、更新检查、媒体库业务接口，以及部分 Jellyfin 兼容媒体接口
- 前端在首次访问或无痕模式没有 token 时，会停留在登录/初始化界面，不再提前加载媒体库选择界面

### 文档

- 说明 `AUTH_USER` / `AUTH_PASS` 用于预置管理员账号；留空时会进入首次创建管理员账号流程
...
- 将“最近观看”入口统一改为“继续观看”
- 调整继续观看入选规则：电影、单集和剧集分集观看超过 1 分钟且未看完时入选，已完全看完的项目不再入选
- 剧集某一季度看完部分分集但未看完整季时，会显示该季度的下一集；继续观看页卡片只保留插旗标记
- 继续观看页封面保持原海报宽度，同时允许高度自适应

### 播放器

- 将播放页换集按钮改为半透明液态玻璃风格的汉堡菜单
- 换集按钮现在跟随 ArtPlayer 控制层一起显示和隐藏

### 品牌与文档

- README 与 wiki 的 LOGO 标识改为新的仓库内文档 LOGO 资源
- 登录页 LOGO 与网页站点图标曾临时改为直接引用远程透明 PNG
- 删除未再引用的前端本地 logo/icon 资源，并清理 README 中失效的详情页截图链接
...
- 将网页 favicon 替换为新的 MediaTree logo 资源
- 将登录页图标替换为新的透明 PNG logo
- README 顶部 logo 引用同步改为仓库内新的 PNG 资源
...
- 本版本标记为需要完整 Docker 镜像更新，因为前端基础镜像、后端运行时依赖锁定、容器运行用户、healthcheck 和 compose 运行配置发生变化
- Release manifest 现在写入 `requires_image_update: true` 和明确原因，设置页会引导到完整镜像更新路径，而不是应用包安装

### 安全与访问控制

- 将静态 bearer 认证替换为签名会话 token，并为视频流、封面、缩略图、字幕、外部播放列表和静态媒体文件增加短期 media token
- 加固远程图片抓取和本地图片读取，只允许安全图片 URL 与预期本地目录
- 加固备份恢复流程，拒绝不安全归档成员、链接、设备文件，以及逃逸 `data_dir` 的 tar 路径
- Docker 自更新命令日志和状态输出会遮蔽敏感环境变量值
- Jellyfin 用户名/密码认证在主应用认证未配置时改为失败关闭

### 初始化与刮削配置

- 修复首次初始化时尚未登录也可以保存 TMDB access token/API 配置
- 设置页保存刮削器/TMDB 配置后会清理前端 config 缓存，手动刮削可立即识别最新 TMDB 状态

### 构建与发布流水线

- 补齐项目级 Codex/ECC 初始化文件 `.agents/`、`.codex/`，并新增 CI 校验 backend tests、backend compile 和 frontend build
- 使用 `backend/constraints.txt` 锁定后端依赖，同时在 `backend/requirements.txt` 保留 `uvicorn[standard]` extras
- Docker 构建更新到 Node 22、Python 3.12 依赖约束、非 root 运行用户和 HTTP healthcheck
