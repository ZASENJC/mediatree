...
- Windows architecture: `windows/MediaTree.Windows/` is the standalone WinUI frontend; `backend/app/` remains the shared MediaTree backend for Web, Docker, and Windows, with Windows-specific behavior limited to `windows_entry.py`, `windows_runtime.py`, packaging, and environment selection. Remote MediaTree, Jellyfin, and Emby connections belong in a WinUI Provider/media-source adapter layer. Do not reuse `backend/app/jellyfin_compat.py` for WinUI outbound Jellyfin/Emby connections; that module is for MediaTree serving Jellyfin/Emby-compatible clients.
- Windows frontend refactor order: split MediaTree API client boundaries first, add Provider contracts and `LocalMediaTreeProvider`, then migrate pages to Provider interfaces. Implement remote MediaTree before Jellyfin/Emby because it matches the existing MediaTree API surface most closely.
