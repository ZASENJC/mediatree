...
test('topbar keeps split liquid glass groups', () => {
  ...
  assert.match(topbar, /className="flex min-w-0 items-center gap-2 liquid-glass topbar-glass topbar-left-glass/)
  assert.match(topbar, /className="flex items-center justify-end gap-1 liquid-glass topbar-glass topbar-right-glass/)
  ...
  assert.match(app, /window\.scrollY > 8/)
  assert.match(app, /window\.requestAnimationFrame\(updateTopbarPosition\)/)
  ...
  assert.match(app, /const setTopbarAwayFromTopWithMeasure = useCallback/)
  assert.match(app, /--mt-topbar-expanded-width/)
  assert.match(app, /Math\.max\(element\.scrollWidth,\s*element\.getBoundingClientRect\(\)\.width\)/)
  assert.match(app, /topbarHovering \|\| topbarFocused \|\| searchOpen \|\| mobileNavOpen \|\| mobileSearchOpen \|\| showLibraryModal/)
  assert.match(app, /const topbarShouldCompact = topbarAwayFromTop && !topbarOpenByInteraction/)
...
test('topbar compact mode animates into two icon spheres', () => {
  ...
  assert.match(css, /\.mt-topbar\.is-compact \.topbar-brand-text,[^}]*\.mt-topbar\.is-compact \.topbar-collapsible\s*\{[^}]*transform:\s*translateX\(-1\.15rem\);/s)
  assert.match(css, /\.mt-topbar\.is-compact \.topbar-right-expanded\s*\{[^}]*transform:\s*translateX\(1\.15rem\);/s)
  ...
  assert.match(compactLogo, /inline-size:\s*1\.45rem;/)
