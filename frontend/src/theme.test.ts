...
test('topbar keeps split liquid glass groups', () => {
  ...
  assert.match(topbar, /to="\/settings"[\s\S]*className=\{`topbar-icon-button relative inline-flex/)
  ...
  assert.match(topbar, /topbar-search-form[\s\S]*<\/form>[\s\S]*className="rounded-full p-1\.5 text-white transition-colors hover:bg-white\/10 sm:p-2 sm:hidden"[\s\S]*<Link[\s\S]*to="\/settings"/)
...
test('home library uses poster grid cards with external title metadata', () => {
  ...
  assert.match(libraryGrid, /data-media-grid-motion="off"/)
  assert.match(homePage, /--mt-home-continue-width:\s*clamp\(var\(--mt-home-continue-min\),\s*var\(--mt-home-continue-fluid\),\s*var\(--mt-home-continue-max\)\);/)
  assert.match(homePage, /--mt-home-poster-min:\s*6rem;/)
  assert.match(homePage, /--mt-home-poster-max:\s*calc\(var\(--mt-home-continue-width\) \* 0\.6667\);/)
  ...
  assert.doesNotMatch(homePage, /auto-fit/)
  ...
  assert.match(css, /@media \(max-width:\s*640px\),\s*\(orientation:\s*portrait\) and \(max-width:\s*820px\)\s*\{[\s\S]*?\.home-page\s*\{[\s\S]*?--mt-home-poster-min:\s*0;[\s\S]*?--mt-home-poster-max:\s*11\.5rem;[\s\S]*?--mt-home-poster-width:\s*min\(var\(--mt-home-poster-max\),\s*var\(--mt-home-poster-three-column-width\)\);[\s\S]*?--mt-media-grid-gap:\s*0\.75rem;/s)
