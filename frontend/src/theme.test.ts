...
test('settings drawer aligns to the topbar and keeps selection weight only on active tabs', () => {
  ...
  const panelInKeyframesStart = css.indexOf('@keyframes settings-drawer-panel-in')
  const panelInKeyframesEnd = css.indexOf('@keyframes settings-drawer-panel-out')
  assert.ok(panelInKeyframesStart >= 0, 'missing settings drawer entrance keyframes')
  assert.ok(panelInKeyframesEnd > panelInKeyframesStart, 'settings drawer entrance keyframes should precede exit keyframes')
  const panelInKeyframes = css.slice(panelInKeyframesStart, panelInKeyframesEnd)
  assert.doesNotMatch(panelInKeyframes, /translateX\(-/)
  assert.doesNotMatch(panelInKeyframes, /scale\(1\.(?:0[1-9]|[1-9])/)
  assert.doesNotMatch(panelInKeyframes, /\n\s*\d+%/)
  assert.doesNotMatch(panelInKeyframes, /filter:/)
...
test('project footer shows GitHub open source attribution inside the settings drawer', () => {
  const app = readFileSync('src/App.tsx', 'utf8')
  const settings = readFileSync('src/pages/settings/Settings.tsx', 'utf8')
  const css = readFileSync('src/index.css', 'utf8')
  const footerRule = cssRuleBody(css, '.mt-project-footer {')
  const footerLinkRule = cssRuleBody(css, '.mt-project-footer-link {')
  const footerIconRule = cssRuleBody(css, '.mt-project-footer-icon {')

  assert.doesNotMatch(app, /mt-project-footer/)
  assert.match(settings, /<footer className="mt-project-footer" aria-label="项目信息">/)
  assert.match(settings, /href="https:\/\/github\.com\/ZASENJC\/mediatree"/)
  assert.match(settings, /target="_blank"/)
  assert.match(settings, /rel="noopener noreferrer"/)
  assert.match(settings, /aria-label="打开 GitHub 项目"/)
  assert.match(settings, /MediaTree · OSS by ZASENJC/)
  assert.doesNotMatch(settings, /Powered by MediaTree · Open Source by ZASENJC/)
  assert.match(settings, /className="mt-project-footer-icon"/)
  assert.match(settings, /<footer className="mt-project-footer" aria-label="项目信息">[\s\S]*<\/footer>\s*<\/div>\s*<aside/)
  assert.match(footerRule, /justify-content:\s*center;/)
  assert.match(footerRule, /padding-block:\s*0\.6rem 0;/)
  assert.match(footerRule, /font-size:\s*0\.72rem;/)
  assert.match(footerLinkRule, /gap:\s*0\.42rem;/)
  assert.match(footerLinkRule, /opacity:\s*0\.62;/)
  assert.match(footerLinkRule, /transition:[^}]*opacity\s+var\(--mt-motion-fast\)\s+ease/s)
  assert.match(footerIconRule, /inline-size:\s*1rem;/)
  assert.match(footerIconRule, /block-size:\s*1rem;/)
  assert.match(footerIconRule, /color:\s*currentColor;/)
  assert.doesNotMatch(footerIconRule, /rgba\(15,\s*23,\s*42/)
  assert.match(css, /\.mt-project-footer-link:hover,[^}]*\.mt-project-footer-link:focus-visible\s*\{[^}]*opacity:\s*0\.9;/s)
})
...
test('detail page fits the player to the viewport with matching side and bottom gutters', () => {
  ...
  const mobileDetailStage = cssRuleBody(css, '.detail-player-stage {', 1)
  const mobileDetailInfoStack = cssRuleBody(css, '.detail-info-stack {', 1)
  ...
  assert.match(detailSource, /const DETAIL_MOBILE_QUERY = '\(max-width: 640px\), \(pointer: coarse\)'/)
  assert.match(detailSource, /function readDetailMobileLayout\(\)/)
  assert.match(detailSource, /function useDetailMobileLayout\(\)/)
  assert.match(detailSource, /const isMobileDetailLayout = useDetailMobileLayout\(\)/)
  assert.match(detailSource, /typeof window === 'undefined' \|\| !window\.matchMedia/)
  assert.match(detailSource, /mediaQuery\.addEventListener\('change', update\)/)
  assert.match(detailSource, /mediaQuery\.addListener\(update\)/)
  ...
  assert.match(detailSource, /fitToContainer=\{!isMobileDetailLayout\}/)
  ...
  assert.match(mobileDetailStage, /display:\s*block;/)
  assert.match(mobileDetailStage, /block-size:\s*auto;/)
  assert.match(mobileDetailInfoStack, /margin-top:\s*0;/)
  assert.match(mobileDetailInfoStack, /padding-top:\s*0;/)
})
